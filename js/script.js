...
function loadComic() {
    ...
    currentSwiper = new Swiper('.comic-swiper', {
        ...
        zoom: {
            maxRatio: 3,
            minRatio: 1,
            toggle: false, // Отключаем зум по двойному тапу, чтобы не мешал жестам
        },
        navigation: {
        ...
        // ЭФФЕКТ INSTAGRAM: Возврат зума после того, как убрали пальцы
        on: {
            touchEnd: function() {
                // Если зум был активен, плавно возвращаем к исходному масштабу
                if (this.zoom) {
                    setTimeout(() => {
                        this.zoom.out();
        ...
        // Остальные параметры для скролла страницы
        keyboard: true,
        autoHeight: true,
