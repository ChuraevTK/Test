...
function loadComic() {
    ...
    allPages.forEach((path, i) => {
        // 1. Создаем слайд (родитель)
        const slide = document.createElement('div');
        slide.className = 'swiper-slide comic-slide';
        
        // 2. Создаем контейнер зума (СТРОГО прямой потомок слайда)
        const zoomContainer = document.createElement('div');
        zoomContainer.className = 'swiper-zoom-container';
        
        // 3. Создаем саму картинку
        const img = document.createElement('img');
        img.src = path;
        img.alt = `Страница ${i+1}`;
        img.loading = 'lazy';
        
        // Собираем матрешку: img -> zoomContainer -> slide
    ...
    currentSwiper = new Swiper('.comic-swiper', {
        slidesPerView: 1,
        centeredSlides: true,
        spaceBetween: 0, // Убираем отступы между слайдами для стабильности зума
        grabCursor: true,
        zoom: {
            maxRatio: 3,
            minRatio: 1,
            toggle: false // Отключаем зум по двойному клику
        },
        navigation: {
        ...
        on: {
            touchEnd: function() {
                if (this.zoom && this.zoom.scale > 1) {
                    setTimeout(() => {
                        this.zoom.out();
                    }, 200);
                }
            },
            // Добавляем явный вызов подгрузки при смене слайда
            slideChange: function() {
                const swiper = this;
                // Проверяем, не пора ли подгрузить следующую страницу/главу
                if (typeof checkScroll === 'function') {
                    checkScroll(); 
                }
                
                // Если ты используешь lazy loading самого Swiper:
                swiper.lazy.load(); 
            },
            // Если подгрузка новой главы происходит в конце списка
            reachEnd: function() {
                console.log("Достигнут конец выпуска, загружаем следующий...");
                if (typeof loadNextChapter === 'function') {
                    loadNextChapter();
                }
            }
        }
